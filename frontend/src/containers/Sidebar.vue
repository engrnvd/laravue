<template>
    <div class="sidebar d-flex" @mouseenter="isMenuOver=true" @mouseleave="isMenuOver=false" @touchstart="isMenuOver=true">
        <div class="main-menu">
                ...
                <ul v-for="(item,itemIndex) in menuItems"
                    ...
                    <li v-for="(sub,subIndex) in item.subs"
                        ...
                        <a v-if="sub.newWindow" :href="sub.to" rel="noopener noreferrer" target="_blank">
                            <i :class="sub.icon + ' sub-menu-icon'"/>
                            <span>{{ sub.label }}</span>
                        </a>
                        <template v-else-if="sub.subs &&  sub.subs.length > 0">
                            <b-link v-b-toggle="`menu_${itemIndex}_${subIndex}`" variant="link"
                                    class="rotate-arrow-icon opacity-50"><i class="simple-icon-arrow-down"></i>
                                <span class="d-inline-block">{{ sub.label }}</span></b-link>
...
export default {
    ...
    mounted() {
        this.menu.send({
            onError: (res) => {
                this.$notify("error", "Error while loading menu", res.data);
            },
            afterRequest: () => {
                this.selectMenu();
            },
    ...
    methods: {
        ...mapMutations('menu', ['changeSideMenuStatus', 'addMenuClassname', 'changeSelectedMenuHasSubItems']),
        selectMenu() {
